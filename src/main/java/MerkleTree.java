...
public class MerkleTree {
    ...
    public static void main(String[] args){
        MerkleTreeTester mtt = new MerkleTreeTester();
        mtt.test();
    }

    public static void sampleImpl(){
        MerkleTree mt = new MerkleTree();
        mt.initTransactions();
        String[] transactionIds = mt.getTransactionIds();
        String merkleRoot = mt.generateMerkleHashRoot(transactionIds);
    ...
    public void initTransactions(){
        ...
        for (int i = 0; i < 10; i ++){
            ...
            transactions.add(t);
            System.out.println("generated TransactionID: " + t.transactionId + " " + t.data.toString());
        }
    }

    public Transaction createTransaction(){
        return new Transaction(new TransactionData(UUID.randomUUID().toString(), UUID.randomUUID().toString(), rand.nextInt(5000) + 1));
    }
    ...
    public String[] getTransactionIds(){
        ...
        return transactionIds.toArray(new String[transactionIds.size()]);
    }

    public String generateMerkleHashRoot(String[] hashList){
        if (hashList.length == 1)
            return hashList[0];
        ArrayList<String> newHashList = new ArrayList<String>();
        for (int i = 0; i < hashList.length-1; i+=2){
            newHashList.add(doubleSHA(hashList[i], hashList[i+1]));
        }

        // it's an odd hash, we need to add this item twice.
        if (hashList.length % 2 == 1) {
            String lastItem = hashList[hashList.length-1];
            newHashList.add(doubleSHA(lastItem, lastItem));
        }

        return generateMerkleHashRoot(newHashList.toArray(new String[newHashList.size()]));
    }
...
class Transaction{
    ...
    public Transaction(TransactionData data){
        this.data = data;
        this.rawTransaction = data.toString().getBytes();
        this.setTransactionId();
    }

    public void setTransactionId(){
        this.transactionId = SHA256Gen.hash(rawTransaction);
    }
}
...
class TransactionData{
    ...
    public TransactionData(String senderId, String receiverId, int amount){
...
class MerkleTreeTester{
    public void test(){
        String[] transactionIds = {"00baf6626abc2df808da36a518c69f09b0d2ed0a79421ccfde4f559d2e42128b",
        ...
        String merkleRoot = mt.generateMerkleHashRoot(transactionIds);
        System.out.println("MerkleRoot: " + merkleRoot);
        System.out.println("Expected:   " + "871714dcbae6c8193a2bb9b2a69fe1c0440399f38d94b3a0f1b447275a29978a");
    }
}